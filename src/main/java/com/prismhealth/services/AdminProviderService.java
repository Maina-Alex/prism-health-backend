...
public class AdminProviderService {
    ...
    public ResponseEntity<?> getProviderById(String phone) {
        ...
        if (user.isPresent()) {
            Users u = user.get();
            u.setRoles(userRolesRepo.findAllByUserId(phone).stream().map(UserRoles::getRole).collect(Collectors.toList()));
    ...
    public List<Users> getAllProviders() {
        ...
        return usersRepo.findAll().stream().filter(u -> Optional.ofNullable(u.getAccountType()).isPresent())
                ...
                .filter(u -> u.getAccountType().equals("PROVIDER")).peek(u -> u.setRoles(userRolesRepo.findAllByUserId(u.getPhone()).stream().map(UserRoles::getRole)
    ...
    public ResponseEntity<?> updateProvider(UpdateProviderRequest req) {
        ...
        if (provider.isPresent()) {
            ...
            if (req.getNewPhone() != null && !req.getNewPhone().equals("")) prov.setPhone(req.getNewPhone());
